...
class MatchWaitView(discord.ui.View):
    ...
    async def accept_match(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.game_type == "blackjack":
            ...
            view = BlackjackPVPView(self.host, interaction.user, self.bet, guild_id)
...
async def blackjack_start(interaction: discord.Interaction, 상대: app_commands.Choice[str], 베팅금액: float):
    ...
    if 상대.value == "bot":
        view = BlackjackBotView(interaction.user, 베팅금액, guild_id)
        embed = view.make_embed()
...
@app_commands.describe(내용="공지 내용", 제목="공지 제목 (선택)")
@app_commands.checks.has_permissions(administrator=True)
async def notice(interaction: discord.Interaction, 내용: str, 제목: str = None):
    title_text = f"📢 {제목}" if 제목 else "📢 공지사항"
    embed = discord.Embed(title=title_text, description=내용, color=discord.Color.blue())
...
@app_commands.describe(내용="업데이트 내용", 제목="업데이트 제목 (선택)")
@app_commands.checks.has_permissions(administrator=True)
async def update(interaction: discord.Interaction, 내용: str, 제목: str = None):
    title_text = f"🛠️ {제목}" if 제목 else "🛠️ 업데이트 공지"
    embed = discord.Embed(title=title_text, description=내용, color=discord.Color.green())
    embed.set_footer(text=f"작성자 : {interaction.user.display_name}", icon_url=interaction.user.display_avatar.url)
    embed.timestamp = discord.utils.utcnow()
    await interaction.response.send_message(embed=embed)
